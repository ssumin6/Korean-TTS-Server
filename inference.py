...
def generate_mels(model, text, pos, removing_silence_mel_padding, adding_silence_mel_padding):
...
def mels_to_wavs_GL(mels, taco_stft, ref_level_db = 0, magnitude_power=1.5):
    ...
    for i, mel in enumerate(mels):
        stime = time.time()
        mel_decompress = mel_denormalize(torch.from_numpy(mel).cuda().unsqueeze(0), hp.max_abs_value)
...
def run(checkpoint_path, sentence_path, clenaer, removing_silence_mel_padding, adding_silence_mel_padding, is_GL, is_melout, is_metaout, output_dir):
    ...
    mels = generate_mels(checkpoint_path, sentences, clenaer, removing_silence_mel_padding, adding_silence_mel_padding, is_GL, output_dir)
    if(is_GL): mels_to_wavs_GL(mels, stft, output_dir)
