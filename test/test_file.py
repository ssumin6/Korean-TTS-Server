...
def synthesis_waveglow(text_seq, model, waveglow, alpha=1.0, mode=""):
    ...
    audio.save_wav(wav[0].data.cpu().numpy(),
                   os.path.join("results_48000", text_seq + mode + ".wav"))
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "t:s:u:w:h",
            ["text=", "step=", "upload=", "wer=", "help"])
    ...
    checkpoint = torch.load(
        os.path.join(hp.checkpoint_path, 'checkpoint_%d.pth.tar' % step_num))
    ...
    while True:
        ...
        if not line: break
        i += 1
        file_name, x, y = synthesis_griffin_lim(line,
                                                model,
                                                alpha=1.0,
                                                mode="normal",
                                                num=step_num,
    ...
    if upload:
        ...
        os.system(
            "gsutil cp -r results_kor_0730_nam_95000/ gs://nm-voice-intern/results_kor_0730_file/"
        )
