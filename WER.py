...
] = "magellan-voice-ui-86932243453a.json"
...
def WERCER(file_paths, org_transcripts):
    ...
    for i, t in enumerate(org_transcripts):
        ...
        cer_diff += wer(
            list(t.replace(" ", "")),
            list(recognized_transcripts[i].replace(" ", "")),
        )
...
def single_aug_experiment():
    ...
    template = (
        "./0_5_aug_100K_test/{}_0_5_{}000_test/wavs/wavenet-audio-mel_{}.wav"
    )
    aug_policy = ["fm", "fw", "tla_d", "tla_s", "tm", "tw", "value"]
...
def datavolume_experiment():
    ...
    template = (
        "./additional_100k_2/none_{}_100000_test/wavs/wavenet-audio-mel_{}.wav"
    )
    datavolumes = ["0_5", "1"]
    ...
    for exp in exps:
        for param in params:
            for num_try in num_trys:
                ...
                file_paths = [
                    "try{}/{}/{}/{}.wav".format(num_try, exp, param, x)
                    for x in range(64)
                ]
                print(WERCER(file_paths, org_transcripts))
